                    ...
                    for (int i = 0; i < gids.Count; i++) {
                        ...
                        if (td.SourceID >= 0) {
                            ...
                            if (map.Orientation == MapOrientation.Orthogonal) {
                                ...
                                td.Target.X = x * mapTiles[td.SourceID].Source.Width + Convert.ToInt32(mapTiles[td.SourceID].Origin.X) + map.Tilesets[mapTiles[td.SourceID].TilesetID].TileOffsetX;
                                td.Target.Y = y * mapTiles[td.SourceID].Source.Height + Convert.ToInt32(mapTiles[td.SourceID].Origin.Y) + map.Tilesets[mapTiles[td.SourceID].TilesetID].TileOffsetY;
                                td.Target.Y += map.TileHeight - td.Target.Height;
